...
try{
 ...
 if(isset($_SESSION["customer"])){
  ...
  $sql=$db->prepare('select * from customer where id!=? and login=?');
  $sql->execute([$id,$_REQUEST['login']]);
}else{
  $sql=$db->prepare('select * from customer where login=?');
  $sql->execute([$_REQUEST['login']]);
}
if(empty($sql->fetch())){
  if(isset($_SESSION["customer"])){
    $sql=$db->prepare('update customer set name=?,login=?,password=? where id=?');
  ...
  $sql=$db->prepare('insert into customer values(null,?,?,?)');
